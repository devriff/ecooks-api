...
class PrivateController Extends CI_Controller {
  protected $layout_view = 'PrivateTemplate';
  ...
  function __construct() {
    parent::__construct();
    $this->view_data['page_title'] = 'CHADZSCRIPT';
  ...
  public function _output ($output) {
    ...
    if ($this->layout_view) {
      echo $this->load->view('Templates/' . $this->layout_view, array(
        'yield' => $yield
      ) , TRUE);
      echo $output;
    }
  ...
  public function _msg ($type = FALSE, $message = FALSE, $redirect = FALSE, $var_name = 'system_message') {
    ...
    switch ($type) {
      ...
      case $type === 's':
      $template = "<div class='uk-alert-success' uk-alert><a class='uk-alert-close' uk-close></a><p>Success! ". $message . "</p></div>";
      break;
